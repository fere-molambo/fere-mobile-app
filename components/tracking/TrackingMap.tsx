...
export default function TrackingMap({ referenceId, referenceType, destination, destinationLat, destinationLng, trackerLabel, height = 250 }: Props) {
  ...
  if ((!isActive || !position) && !resolvedDestination) {
    return (
      <View style={[styles.container, { height: height * 0.6 }]}>
        <View style={styles.placeholderContent}>
          <MapPin color="#999" size={24} />
          <Text style={styles.placeholderText}>Suivi en temps réel non disponible pour le moment</Text>
        </View>
      </View>
    );
  }

  const dist = position && resolvedDestination ? calculateDistance(position.lat, position.lng, resolvedDestination.lat, resolvedDestination.lng) : null;
  ...
  return (
    <View style={styles.container}>
      <View style={[styles.mapWrapper, { height }]}>
        <LeafletMap position={position} destination={resolvedDestination} />
        {!position && (
          <View style={styles.waitingOverlay}>
            <Text style={styles.waitingText}>En attente de la position en direct...</Text>
          </View>
        )}
      ...
      <View style={styles.infoBar}>
        <View style={styles.infoItem}>
          ...
          <Text style={styles.infoValue}>{position ? 'En direct' : 'Hors ligne'}</Text>
        </View>
        {distText && (
          <View style={styles.infoItem}>
...
const styles = StyleSheet.create({
  ...
  waitingOverlay: { position: 'absolute', bottom: 8, left: 8, backgroundColor: 'rgba(0,0,0,0.6)', borderRadius: 6, paddingHorizontal: 8, paddingVertical: 4 },
  waitingText: { color: '#fff', fontSize: 11 },
