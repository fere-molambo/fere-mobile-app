...
export default function CheckoutScreen() {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      // === Création des commandes en DB (miroir du flux web Checkout.tsx) ===
      const tvaRate = (platformFees.tva_rate || 18) / 100;
      const driverEarningsPercent = (platformFees.delivery_commission_driver || 80) / 100;
      const createdOrders: any[] = [];

      for (const so of summary.shopOrders) {
        const shopSubtotal = so.subtotal;
        const shopDeliveryFee = so.deliveryFee;
        const shopDeliveryCommission = so.deliveryCommission;
        const shopProductCommission = so.productCommission;
        const shopTotal = shopSubtotal + shopDeliveryFee;
        const shopTva = Math.round(shopSubtotal * tvaRate);
        const shopAdvance = shopDeliveryFee + shopDeliveryCommission + shopProductCommission;
        ...
        const { data: order, error: orderError } = await supabase
          ...
          .insert({
            ...
            tva_amount: shopTva,
            commission_amount: shopProductCommission,
        ...
        // Insertion items
        const orderItems = so.items.map((item: any) => {
          const rate = (item.product.category_id && commissionRates.get(item.product.category_id))
            ?? so.productCommissionRate ?? 10;
          const itemTotal = (item as any).totalPrice ?? (item.quantity * item.unitPrice);
          return {
            order_id: order.id,
            ...
            commission_amount: Math.round(itemTotal * (rate / 100)),
        ...
        // Insertion delivery_request
        const driverEarnings = Math.round(shopDeliveryFee * driverEarningsPercent);
        const pickupPoint = {
          shop_id: so.shop.id,
          shop_name: so.shop.name,
          lat: so.shop.geolocation_lat || 0,
          lng: so.shop.geolocation_lng || 0,
          address: so.shop.address || '',
        };
        const { error: deliveryError } = await supabase
          .from('delivery_requests')
          .insert({
            order_id: order.id,
            zone_id: so.shop.delivery_zone_id || null,
            status: 'pending',
            pickup_point: pickupPoint,
            pickup_points: [pickupPoint],
            delivery_point: {
              lat: deliveryAddr?.geolocation_lat,
              lng: deliveryAddr?.geolocation_lng,
              address: deliveryAddr?.address,
              recipient_name: deliveryAddr?.recipient_name || (user as any).user_metadata?.nom_complet,
              recipient_phone: deliveryAddr?.recipient_phone,
            },
            delivery_fee: shopDeliveryFee,
            driver_earnings: driverEarnings,
          });
        if (deliveryError) throw new Error(`Échec création livraison: ${deliveryError.message}`);

        createdOrders.push(order);
      }
      // === Fin création des commandes ===
      ...
      const omBody = {
        ...
        metadata: {
          ...
          payment_group_id: paymentGroupId,
          user_id: user.id,
          order_numbers: createdOrders.map((o: any) => o.order_number).join(', '),
